export const updateTask = async (task_lessonId, task_id, name, description, rightAnswer) => {
  try {
    const response = await fetch(`http://localhost:8080/task/update/${task_id}`, {
      ...
      body: JSON.stringify({ name: name, description: description, lessonId: task_lessonId, rightAnswer: rightAnswer}),
    });
